...
export const rpc_getraw_transaction_withvin = async function (
  config: ConfigService,
  walletName: string,
  txid: string,
): Promise<any | Error> {
...
export const rpc_scanxoutset = async function (
  config: ConfigService,
  descriptor: string,
): Promise<any | Error> {
  const Authorization = `Basic ${Buffer.from(
  ...
  console.log(`Scanning UTXOs for descriptor: ${descriptor}`);

  const blockchainInfo = await rpc_getblockchaininfo(config);
  const latestBlockHeight = blockchainInfo.blocks;
  const startBlock = Math.max(latestBlockHeight - 1, 0);
  ...
  return fetch(config.getRpcUrl(null), {
    ...
    body: JSON.stringify({
      ...
      params: [
        ...
        [
          {
            desc: descriptor,
            range: [startBlock, latestBlockHeight],
          },
        ],
      ],
    }),
    ...
    .then((res) => {
      ...
      if (contentType.includes('json')) {
        ...
        throw new Error(
          `Invalid content type: ${contentType}, status: ${res.status}`,
        );
      }
    })
    .then((res: any) => {
      if (res.error) {
        console.log('Response from scantxoutset:', res);
    ...
    .catch((e: Error) => {
      console.error('RPC scantxoutset error:', e);
      return e;
    });
};

export const rpc_getblockchaininfo = async function (
  config: ConfigService,
): Promise<any | Error> {
  const Authorization = `Basic ${Buffer.from(
    `${config.getRpcUser()}:${config.getRpcPassword()}`,
